function [bestmem , bestval, nfeval , iter] = o_devec6 ( fname , VTR , D ,...
...
parfor i = 1:NP   % check the remaining members
    restoreEnvironment(environment);
    val(i) = feval(fname, pop(i,:), y);
end

fprintf('First Pop is completed :) \n \n')
worstval = max(val);
...
while ((iter < itermax) && (abs(bestval - worstval) > VTR))
    ...
    fprintf('Itr is completed :) \n \n')
    val(tempval <= val) = tempval(tempval <= val);
    pop(tempval <= val,:) = ui(tempval <= val,:);
    worstval = max(val);
    [bestval, ibest] = min(val);
    bestmem = pop(ibest, :);
